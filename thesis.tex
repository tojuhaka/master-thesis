...
\section{Historia}
MVC:n esitteli Norjalainen Trygve Reenskaug  Xerox PARC -tutkimushankkeessa.
...
Tässä osiossa tarkastellaan Pyramid-sovelluksen tiedostoja sekä niiden sisältöjä MVC-kom"-ponenttien näkökulmasta. Erityisesti keskitytään Ohjaimen ja Näkymän toteutukseen. Samalla tutkitaan, voidaanko Pyramid havaintojen perusteella luokitella MVC:n toteuttavaksi sovelluskehykseksi.
...
\chapter{Tulokset}
Internetin arkkitehtuuri koostuu erilaisista kerroksista protokollia. Web-sovellukset rakennetaan protokollien ylimmälle tasolle, jossa on vapaus toteuttaa sovellus sellaisena kuin sovelluksen kehittäjät näkevät parhaaksi. Tästä syystä mahdolliset esteet arkkitehtuurin toteutukselle tulee purkaa alempien kerroksien protokollista lähtien. MVC:n alkuperäinen toteutus vaatii kaksisuuntaista kommunikaatiota, jolloin kommunikaatio-ongelma on ratkaistava alemman tason protokollissa. TCP- ja IP-protokolla pitävät huolen yhteyden muodostamisesta ja tiedon siirtämisestä sovelluksien välillä, mutta eivät rajoita millään tavalla kommunikaation suuntaa. MVC-arkkitehtuurille ei siis ole rajoituksia TCP- ja IP-protokollassa. Tällöin voidaan siirtyä kerroksissa ylemmäksi, jolloin vastaan tulee HTTP-protokolla, jonka päälle useimmat web-sovellukset on rakennettu. HTTP-protokollan luonne kuitenkin rajoittaa kommunikaatiota sovelluskomponenttien välillä yksisuuntaiseksi.  Voidaan siis väittää, että kaikki HTTP:n päälle rakennetut web-sovellukset eivät toteuta MVC:tä alkuperäisen määritelmän mukaisesti. Vaikka Django ja Pyramid määritellään MVC-sovelluskehyksiksi, eivät ne toteuta MVC:n määritelmää. Kummankin MVC-toteutuksessa kommunikaatio Mallin ja Näkymän välillä toteutuu ohjaimen kautta, vaikka alkuperäisessä määrittelyssä myös Mallin tulisi olla suoraan yhteydessä Näkymään. Lisäksi vaaditaan kaksisuuntainen kommunikaatio komponenttien välillä ja tämä ei toteudu HTTP:n päälle rakennetuissa web-sovelluksissa. 

Web-soketti -protokollan päälle rakennetuissa web-sovelluksissa mahdollistetaan kaksisuuntainen kommunikaatio, jolloin MVC:n toteutuksen esteeksi ei tule sovelluskerroksen tiedonsiirtoprotokolla toisin kuin HTTP:n kanssa. Tällöin tiedonsiirtoprotokollien yläpuolella oleva sovelluksen arkkitehtuurillinen rakenne on vapaasti määriteltävissä ja mahdollistaa MVC:n toteutuksen alkuperäisen määritelmän mukaisesti ilman alempien kerroksien teknisiä esteitä. MVC-arkkitehtuuri on toteutettavissa Tornadolla, koska Tornado ottaa hyvin kevyesti kantaa sovelluksen arkkitehtuuriin sekä mahdollistaa sovelluksen toteutuksen web-sokettien päälle. Tästä syystä Tornadon ja web-sokettien avulla voidaan toteuttaa MVC alkuperäisen määritelmän mukaisesti. 
